...
new_quantization_matrix = np.array(
    [
...
def grayscale_jpeg_encoder(
    img: np.ndarray[np.uint8], block_size: int, num_coefficients: int, quantization_matrix: np.ndarray[np.float32]
) -> list[np.ndarray[np.int32]]:
    """
...
def grayscale_jpeg_decoder(
    blocks: list[np.ndarray[np.int32]], img: np.ndarray[np.uint8], block_size: int, quantization_matrix: np.ndarray[np.float32]
) -> np.ndarray[np.uint8]:
    """
...
def color_jpeg_encoder(
    ...
    return (
        grayscale_jpeg_encoder(blue_channel, block_size, num_coefficients, quantization_matrix),
        grayscale_jpeg_encoder(green_channel, block_size, num_coefficients, quantization_matrix),
        grayscale_jpeg_encoder(red_channel, block_size, num_coefficients, quantization_matrix),
    )
...
def color_jpeg_decoder(
    ...
    block_size: int, quantization_matrix: np.ndarray[np.float32]
) -> np.ndarray[np.uint8]:
    """
    ...
    blue_channel = grayscale_jpeg_decoder(blocks[0], blue_channel, block_size, quantization_matrix)
    green_channel = grayscale_jpeg_decoder(blocks[1], green_channel, block_size, quantization_matrix)
    red_channel = grayscale_jpeg_decoder(blocks[2], red_channel, block_size, quantization_matrix)
...
def jpeg_encoder(
    ...
    color: bool, quantization_matrix: np.ndarray[np.float32]
) -> (
    list[np.ndarray[np.int32]]
    | tuple[
        list[np.ndarray[np.int32]],
    ...
    if color:
        ...
        return color_jpeg_encoder(img, block_size, num_coefficients, quantization_matrix)
    else:
        # Load grayscale image and apply grayscale JPEG encoder
        img = cv.imread(img_path, cv.IMREAD_GRAYSCALE)
        return grayscale_jpeg_encoder(img, block_size, num_coefficients, quantization_matrix)
...
def jpeg_decoder(
    ...
    if color:
        img = cv.imread(img_path, cv.IMREAD_COLOR)
        return color_jpeg_decoder(blocks, img, block_size, quantization_matrix)
    else:
        img = cv.imread(img_path, cv.IMREAD_GRAYSCALE)
        return grayscale_jpeg_decoder(blocks, img, block_size, quantization_matrix)


def analyze_image(
    img_path: str, block_size: int, num_coefficients: int, color: bool, quantization_matrix: np.ndarray[np.float32]
) -> tuple[
    np.ndarray[np.uint8],
    np.ndarray[np.uint8],
    float,
    ...
    encoded_img = jpeg_encoder(img_path, block_size, num_coefficients, color, quantization_matrix)

    # Decode the image using JPEG compression
    compressed_img = jpeg_decoder(encoded_img, img_path, block_size, color, quantization_matrix)
...
#     """
#
#     img_path = "/home/ilker/PycharmProjects/jpeg-firat/pnomoni/image-0.jpg"
#     block_size = 8
#     num_coefficients = 10
#     color = "y"
#     plot_images(*analyze_image(img_path, block_size, num_coefficients, color, new_quantization_matrix))
